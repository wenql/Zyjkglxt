...
namespace TcmHMS.Tests
{
    public abstract class TcmHMSTestBase : AbpIntegratedTestBase<TcmHMSTestModule>
    {
        ...
        protected void UsingDbContext(Action<TcmHMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TcmHMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TcmHMSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TcmHMSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TcmHMSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TcmHMSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TcmHMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TcmHMSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TcmHMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TcmHMSDbContext, Task<T>> func)
        {
            T result;
